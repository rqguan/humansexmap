...
- **标签字体随放大增长**：缩放 ≤200%（`Z_FONT_LOCK`）时字号恒定；超过后随地图等比放大（450% 时约 2.25×），避免高倍视角下地图空旷、文字过小。
- **注解并入点击循环（移动端）**：点按词条任意位置 = 循环选择状态 + 弹出注解（桌面端仍为悬停注解、点击循环）。
- **移动端初始视图改为整图概览**：进入即可见大区名称文字（桌面端保持标题区锚定 90% 不变）。
- **平移瓦片加载**：`updateWhenIdle:false` + `keepBuffer:4`，平移过程中即时补载瓦片，消除滑动时的空白/加载滞后。
